...
def context_fusion(
    ...
    """
    Context Fusion: Merge events + entities from logs and chat + commands into one incident representation.
    """
...
def validate_extraction(incident_json: Dict[str, Any], raw_logs: str, raw_chat: str) -> List[str]:
    ...
    # To check if extracted entities actually appear in the Raw logs/chat (basic sanity check)
    ...
    _must_appear(incident_json.get("services_affected", []), "Service") # if these services in JSON, they should appear in logs/chat
